...
def save_glm(
    ...
    table_stand_beta_con.to_csv(
        os.path.join(out_p, f"{base_filename}_standardized_betas.tsv"),
        sep="\t",
    )
    table_qval_con.to_csv(
        os.path.join(out_p, f"{base_filename}_fdr_corrected_pvalues.tsv"),
        sep="\t",
    )

    print(f"\n✅ Completed processing for feature: {feature}")
    print(
        f"✅ Results saved to: {os.path.join(out_p, f'{base_filename}.tsv')}"
    )
...
def summarize_glm(glm_table, conn_mask, roi_labels):
    ...
    stand_beta_table = pd.DataFrame(
        conn2mat(out_table.stand_betas.values, conn_mask),
        index=roi_labels,
        columns=roi_labels,
    )
    qval_table = pd.DataFrame(
        conn2mat(out_table.pvals.values, conn_mask),
        index=roi_labels,
        columns=roi_labels,
    )
...
def glm_wrap_cc(
    out_p, conn, pheno, group, case, control, regressors="", report=False
):
    # Make sure pheno and conn have the same number of cases
    if not conn.shape[0] == pheno.shape[0]:
        print(
            f"❌ Connectivity matrix ({conn.shape[0]})"
            f"and phenotype file ({pheno.shape[0]})"
    ...
    if report:
        summary_data = {
            "Selected sample based on group variable": f"{group}",
            f"cases {case}": f"n={n_case}",
            f"controls {control}": f"n={n_control}",
            "original sample": f"n={pheno.shape[0]}",
            "new sample": f"n={n_sub}",
            "data points available": f"{n_data}",
        ...
        for key, value in summary_data.items():
            if isinstance(value, list):
                print(
                    f"{key}: {len(value)}"
                )  # Or print all items if preferred
            else:
                print(f"{key}: {value}")
