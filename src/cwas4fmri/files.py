...
def bids_validation(bids_dir):
    ...
    if not bids_dir.exists():
        raise FileNotFoundError(
            f"❌ Invalid path to BIDS directory: {bids_dir}"
        )

    if not (bids_dir / "dataset_description.json").exists():
        raise FileNotFoundError(
            f"❌ Missing dataset_description.json in BIDS directory: {bids_dir}"
            "🔗 Your files should follow the BIDS BED-017 specification. \n"
            "For more information: https://bids.neuroimaging.io/"
        )

    if not (bids_dir / "meas-PearsonCorrelation_relmat.json").exists():
        raise FileNotFoundError(
            f"❌ Missing meas-PearsonCorrelation_relmat.json in {bids_dir}"
            "🔗 Your files should follow the BIDS BED-017 specification. \n"
            "For more information: https://bids.neuroimaging.io/"
        )
...
def find_bids_output(working_directory):
    ...
    connectome_t = os.path.join(
        "{}",
        "ses-{}",
        "func",
        "{}_ses-{}_task-{}_run-{}_seg-{}"
        "_meas-PearsonCorrelation_desc-{}_relmat.tsv",
    )
...
def verify_working_directory(working_directory):
    ...
    if not os.path.exists(working_directory):
        raise FileNotFoundError(
            f"❌ Invalid path to BIDS directory: {working_directory}"
        )


def verify_exclude_json(json_exclude_qc_path, reports_dir):
    print("\n⏳ Verifying exclude.json file location ...")
    ...
    if not os.path.exists(exclude_file):
        raise FileNotFoundError(
            f"❌ Missing exclude.json in reports directory: {exclude_file}"
        )
...
def verify_files_and_directories(
    working_directory, json_exclude_qc_path, reports_dir, out_p
):
    """
