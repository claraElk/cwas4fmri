...
def read_phenotype_file(path):
    ...
    if ext not in [".tsv", ".csv", ".xlsx"]:
        raise ValueError(
            f"❌ Unsupported extension: {ext}. Expected .tsv, .csv, or .xlsx\n"
        )
    try:
        if ext == ".tsv":
...
def validate_subject_ids(df, subject_file_path):
    ...
    if not invalid.empty:
        warnings.warn(
            "❗️ Found invalid subject IDs:"
            f"{invalid['participant_id'].tolist()[:5]} \n"
        )
    return df
...
def encode_diagnosis(df, case, control):
    ...
    if not invalid.empty:
        warnings.warn(
            "❗️ Unexpected diagnosis values:"
            f"{invalid['diagnosis'].unique()}\n"
        )
...
def warn_on_invalid_age(df):
    invalid = df[
        ~df["age"].apply(lambda x: isinstance(x, (int, float)) and x > 0)
    ]
    if not invalid.empty:
        warnings.warn(
            "❗️ Possible invalid age values in rows:"
            f"{invalid.index.tolist()[:5]}\n"
        )
