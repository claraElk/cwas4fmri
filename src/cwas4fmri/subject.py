...
def find_valid_subjects(
    bids_dir, pheno, session, connectome_t, run, task, atlas, feature, out_p
):
    print("⏳ Identify subjects connectivity matrix ...")
    print("This will take a moment, please do not interrupt the process ...\n")
    ...
    return pheno[pheno["participant_id"].isin(processed_subjects)].reset_index(
        drop=True
    )
...
def find_subset(pheno, column, cases=None):
    ...
    if cases is not None and not not cases:
        ...
        try:
            ...
            raise Exception(
                '❌ the attribute "cases" needs to be iterable'
                "but is: {type(cases)}"
            )
        ...
        if not all(case_available):
            if not any(case_available):
                raise Exception(
                    f"❌ none of the requested cases {cases}"
                    f'of "{column}" are available'
                )
            else:
                warnings.warn(
