...
def plot_interactive_matrix(output_path, beta_matrix, pvalues_matrix, labels):
    ...
    significance_mask[upper_triangle_indices] = (
        pvalues_corrected[upper_triangle_indices] < 0.05
    )
    ...
    mask = np.where(
        significance_mask, 1.0, np.nan
    )  # show only where significant
    ...
    fig.update_layout(
        title="Interactive beta value heatmap with FDR correction",
