...
def filter_by_qc(json_file_path, pheno_filtered, out_p):
    """
    Process ratings and clean phenotype table
    by removing subjects with bad ratings
    """
    print("⏳ Identify subjects with bad QC ...")

    # Dictionary to store subject ratings
    subject_ratings = defaultdict(
        lambda: {"good": 0, "bad": 0, "uncertain": 0}
    )
    ...
    print("✅ Information saved in:", json_path)
...
def filter_by_fd(
    ...
    print(
        "This might take a moment, please do not interrupt the process ...\n"
    )
    ...
    for _, row in tqdm(pheno_filtered_qc.iterrows()):
        json_file_path = os.path.join(
            ...
            confounds_json.format(
                row["participant_id"], session, task, run, feature
            ),
        )
    ...
    pheno_filtered_fd_mean = pheno_filtered_qc[
        pheno_filtered_qc["mean_fd"] < 0.5
    ]
    subjects_with_mean_rejection = pheno_filtered_qc[
        pheno_filtered_qc["mean_fd"] > 0.5
    ]["participant_id"]
