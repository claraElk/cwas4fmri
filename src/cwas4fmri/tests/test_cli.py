...
def create_dummy_phenotype(bids_dir, subject_ids):
    ...
    with open(phenotype_file, "w") as f:
        f.write(
            "participant_id\tdiagnosis\tsex\tage"
            "\tmedication\tsequence\tscanner\n"
        )
        for sub in subject_ids:
            ...
            f.write(
                f"{sub}\t{diagnosis}\t{sex}\t{age}"
                f"\t{medication}\t{sequence}\t{scanner}\n"
            )
...
def create_bids_dir_structure(bids_dir):
    ...
    with open(
        os.path.join(bids_dir, "meas-PearsonCorrelation_relmat.json"), "w"
    ) as f:
        json.dump(meas_json, f, indent=4)


def create_dummy_json(bids_dir, sub):
    confounds_json = os.path.join(
        "{}_ses-{}_task-{}_run-{}_desc-{}_timeseries.json"
    )
    ...
    json_file = os.path.join(
        ...
        "ses-timepoint1",
        "func",
        confounds_json.format(
            sub, "timepoint1", "task01", "01", "denoiseSimple"
        ),
    )
    info = {
        "ConfoundRegressors": [
...
def create_dummy_data(bids_dir):
    ...
    for sub in sub_list:
        sub_dir = os.path.join(
            bids_dir, connectome_t.format(sub, "timepoint1")
        )
        os.makedirs(sub_dir, exist_ok=True)
        sub_connectome = (
            f"{sub}_ses-timepoint1_task-task01_run-01_seg-example_atlas"
            "_meas-PearsonCorrelation_desc-denoiseSimple_relmat.tsv"
        )
...
def test_smoke(tmpdir):
    ...
    assert os.path.isfile(
        os.path.join(
            output_dir,
            "cwas_NDD_HC_rsfmri_denoiseSimple"
            "_example_atlas_fdr_corrected_pvalues.tsv",
        )
    ), "Pval file was not created"
    assert os.path.isfile(
        os.path.join(
            output_dir,
            "cwas_NDD_HC_rsfmri_denoiseSimple"
            "_example_atlas_standardized_betas.tsv",
        )
    ), "Beta file was not created"
    assert os.path.isfile(
        os.path.join(
            output_dir, "cwas_NDD_HC_rsfmri_denoiseSimple_example_atlas.tsv"
        )
    ), "TSV file was not created"
    assert os.path.isfile(
        os.path.join(output_dir, "cwas_report.json")
