...
def workflow(args):
    ...
    Args:
        ...
        cat_covariates (str): comma-separated list of categorical covariates to include in the model
        num_covariates (str): comma-separated list of numerical covariates to include in the model
    ...
    logger.info(f"Run CWAS analysis for : \n-strategy {args.strategy} and \n-atlas: {args.atlas}"
    ...
    # Save results
    base_filename = f'cwas_{args.patient}_{args.control}_rsfmri_{args.strategy}_{args.atlas}'
    table_con.to_csv(output / f'{base_filename}.tsv', sep='\t')
    table_stand_beta_con.to_csv(output / f'{base_filename}_standardized_betas.tsv', sep='\t')
    table_qval_con.to_csv(output / f'{base_filename}_fdr_corrected_pvalues.tsv', sep='\t')
    table_pval_con.to_csv(output / f'{base_filename}_pvalues.tsv', sep='\t')
    table_beta_con.to_csv(output / f'{base_filename}_betas.tsv', sep='\t')
    
    logger.info(f"Completed processing for the following strategy: {args.strategy}")
